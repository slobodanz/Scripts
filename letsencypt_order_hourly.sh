...
ATO_NETS="62.20.146.242/32 62.20.146.241/32"
certbot="/usr/bin/certbot"
wanted_cert_path="/etc/letsencrypt/live"
premium_cert_path="/appdir/atomia/ssl/premium_certificates"
synced_apache_config="/storage/configuration/maps"
synced_apache_config_cur="/appdir/atomia/synced_apache_config_cur"
...
while [  $TIME -ge 5 ]; do
 NEXT=$((TIME+5))
 cp $synced_apache_config_cur/vhost.map.$TIME $synced_apache_config_cur/vhost.map.$NEXT
 TIME=$((TIME-5))
done

grep -Fxvf $synced_apache_config_cur/vhost.map $synced_apache_config/vhost.map > $synced_apache_config_cur/vhost.map.5

#merge in one diff file
cat "$synced_apache_config_cur/vhost.map.5" > $synced_apache_config_cur/vhost.map.diff

TIME=10
while [  $TIME -le 120 ]; do
 cat "$synced_apache_config_cur/vhost.map.$TIME" >> $synced_apache_config_cur/vhost.map.diff
 TIME=$((TIME+5))
done

cp -p $synced_apache_config/vhost.map $synced_apache_config_cur/vhost.map
...
if [ -f "$synced_apache_config_cur/vhost.map.diff" ]; then
        cat "$synced_apache_config_cur"/vhost.map.diff | awk '{ print $1 }' | grep -vE "$1"'$' | grep -v '^www\.' | grep -E '^[a-zA-Z0-9.-]+$' \
                ...
                if [ ! -d `echo "$wanted_cert_path/$desired"` ] && [ ! -d `echo "$premium_cert_path/$desired"` ]; then
                        ...
                        ip2="$(dig +short $wanted_cert | head -1)"
                                ...
                                if  [ -n "$ip1" ] && [ "$(in_ato_nets $ip1)" = yes ] && [ -n "$ip2" ] && [ "$(in_ato_nets $ip2)" = yes ] ; then
                                        ...
                                        $f5hook add letsencrypt $wanted_cert linux
                                fi
                fi
        done
fi
                ...
                if [ ! -d `echo "$wanted_cert_path/$desired"` ] && [ ! -d `echo "$premium_cert_path/$desired"` ]; then
                        ...
                        ip2="$(dig +short $wanted_cert | head -1)"
                                if  [ -n "$ip1" ] && [ "$(in_ato_nets $ip1)" = yes ] && [ -n "$ip2" ] && [ "$(in_ato_nets $ip2)" = yes ] ; then
                                        ...
                                        #Upload certificate to F5
                                        $f5hook add letsencrypt $wanted_cert windows
                                fi
                fi
        done
fi
